...
void Hull::AddVertex(const Vec3Physics& v)
{
	HullVertex new_vertex;
	new_vertex.idx = m_Vertices.size();
	new_vertex.pos = v;
...
int Hull::ConstructNewEdge(int parent_face_idx, int vert_start, int vert_end)
{
	...
	if (out_idx == -1)
	{
		out_idx = m_Edges.size();

		HullEdge new_edge;
		new_edge.idx = m_Edges.size();
...
void Hull::AddFace(const Vec3Physics& normal, int nVerts, const int* verts)
{
	HullFace new_face;
...
void Hull::GetMinMaxVerticesInAxis(const Vec3Physics& local_axis, int* out_min_vert, int* out_max_vert)
{
	float cCorrelation;
	...
	{
		cCorrelation = local_axis.Dot(m_Vertices[i].pos);
...
void Hull::DebugDraw(const Mat4Physics& transform)
{
	...
	for (HullFace& face : m_Faces)
	{
		...
		{
			Vec3Physics polygon_start = transform * m_Vertices[face.vert_ids[0]].pos;
			Vec3Physics polygon_last = transform * m_Vertices[face.vert_ids[1]].pos;

			for (size_t idx = 2; idx < face.vert_ids.size(); ++idx)
			{
				Vec3Physics polygon_next = transform * m_Vertices[face.vert_ids[idx]].pos;

				NCLDebug::DrawTriangle(polygon_start, polygon_last, polygon_next, Vec4Graphics(1.0f, 1.0f, 1.0f, 0.2f));
	...
	{
		NCLDebug::DrawThickLine(transform * m_Vertices[edge.vStart].pos, transform * m_Vertices[edge.vEnd].pos, 0.02f, Vec4Graphics(1.0f, 0.2f, 1.0f, 1.0f));
	}
}