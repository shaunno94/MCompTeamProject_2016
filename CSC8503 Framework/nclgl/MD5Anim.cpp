...
void MD5Anim::LoadMD5AnimBaseFrame( std::ifstream &from )	{
	...
	*/
	baseFrame.orientations	= new QuatGraphics[numJoints];
	baseFrame.positions		= new Vec3Graphics[numJoints];
	...
	do {
		...
		if(tempLine == "{") {			//In a well-behaved MD5 file, the first line will be '{'
			...
			QuatGraphics is left out of the files. As we're dealing with unit length 
			QuatGraphicss (i.e they have a length of 1), the 4th component will be 
					sqrt of (1.0 - length of the other 3 components)
			*/

			baseFrame.orientations[current].GenerateW();
			baseFrame.orientations[current].Normalize();
...
void	MD5Anim::TransformSkeleton(MD5Skeleton &skel, unsigned int frameNum) {
	...
	*/
	...
	for(unsigned int i = 0; i < numJoints; ++i) {
		...
		Vec3Graphics		animPos	 = baseFrame.positions[i];
		QuatGraphics  animQuat = baseFrame.orientations[i];
		...
		*/
		...
		animQuat.Normalize(); //And we should probably normalise it, too, to keep to unit length
		...
		//QuatGraphics into a Matrix4, then we set the resulting matrix translation to the
		//transformed baseframe position

		skelJoint.localTransform		= animQuat.ToMatrix4();
		skelJoint.localTransform.SetTranslation(animPos);
