...
once as separate Vec3Graphics and QuatGraphics (loaded from the MD5Mesh file) and
once as a Mat4Graphics, generated at run-time. This is to make it more obvious
...
struct MD5Joint {
	...
	int			forceWorld;
	Vec3Graphics		position;		//Position relative to parent joint
	QuatGraphics	orientation;	//Orientation relative to parent joint
	Mat4Graphics		transform;		//World transform of this joint

	Mat4Graphics		localTransform;
};
...
struct MD5Vert {
	...
	Vec2Graphics texCoords;		//Texture coordinates of this vertex
...
struct MD5Weight {
	...
	Vec3Graphics position;		//Anchor position of this MD5Weight
};
...
//functions to upload that data to the graphics card...
	...
	*/
	static const Mat4Graphics conversionMatrix;

	static const Mat4Graphics inverseConversionMatrix;
...
//get their data from, for consistency with other VBO data arrays we make
	...
	Mat4Graphics*		transforms;			//Array of skeleton transforms
	Vec3Graphics*		weightings;			//Array of Vertex weightings
#endif
