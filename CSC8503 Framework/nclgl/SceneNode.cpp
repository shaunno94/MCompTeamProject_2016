...
SceneNode::SceneNode(Mesh*mesh, const Vec4Graphics& colour)	{
	awake				= true;
	this->mesh			= mesh;
	this->colour		= colour;
	parent				= NULL;
...
bool	SceneNode::CompareByZ(SceneNode*a,SceneNode*b)  {
	return (a->GetWorldTransform().GetTranslation().z < b->GetWorldTransform().GetTranslation().z) ? true : false;
}
