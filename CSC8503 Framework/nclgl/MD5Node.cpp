...
bool	MD5Node::GetJointWorldTransform(const string&name, Mat4Graphics &t) {
...
bool	MD5Node::GetJointLocalTransform(const string&name, Mat4Graphics &t) {
	int index = sourceData.GetIndexForJointName(name);
	if (index < 0) {
		return false;
	}
...
bool	MD5Node::GetParentLocalOrientation(const string&name, QuatGraphics &t) {
	int index = sourceData.GetIndexForJointName(name);
	if (index < 0) {
		return false;
	}
...
bool	MD5Node::GetParentWorldOrientation(const string&name, QuatGraphics &t) {
	return false;
}


bool	MD5Node::GetParentLocalTransform(const string&name, Mat4Graphics &t) {
	int index = sourceData.GetIndexForJointName(name);
	if (index < 0) {
		return false;
	}
...
bool	MD5Node::GetParentWorldTransform(const string&name, Mat4Graphics &t) {
	int index = sourceData.GetIndexForJointName(name);
	if (index < 0) {
		return false;
	}
...
bool	MD5Node::SetJointLocalTransform(const string &name, Mat4Graphics &t) {
	int index = sourceData.GetIndexForJointName(name);
	if (index < 0) {
		return false;
	}
...
bool	MD5Node::SetJointWorldTransform(const string &name, Mat4Graphics &t) {
	int index = sourceData.GetIndexForJointName(name);
	if (index < 0) {
		return false;
	}
...
void	MD5Node::DebugDrawSkeleton() {
		...
		Vec3Graphics*	 skeletonVertices = new Vec3Graphics[currentSkeleton.numJoints*2];
		...
		*/
		for(int i = 0; i < currentSkeleton.numJoints; ++i) {
			skeletonVertices[i*2] = currentSkeleton.joints[i].transform.GetTranslation();
	
			if(currentSkeleton.joints[i].parent == -1) {	//No parent, but to keep this simple we'll copy the position again...
				skeletonVertices[(i*2)+1] = currentSkeleton.joints[i].transform.GetTranslation();;
			}
			else{
				skeletonVertices[(i*2)+1] = currentSkeleton.joints[currentSkeleton.joints[i].parent].transform.GetTranslation();;
			}
		}
		...
		glBufferData(GL_ARRAY_BUFFER, currentSkeleton.numJoints*sizeof(Vec3Graphics) * 2, skeletonVertices, GL_STREAM_DRAW);
...
void	MD5Node::DebugDrawJointTransforms(float size, bool worldSpace) {
	...
	Vec3Graphics*	 skeletonVertices = new Vec3Graphics[numVerts];
	Vec4Graphics*	 skeletonColours  = new Vec4Graphics[numVerts];


	for (int i = 0; i < currentSkeleton.numJoints; ++i) {
		Mat4Graphics transform = (worldSpace ? currentSkeleton.joints[i].transform : currentSkeleton.joints[i].localTransform);

		Vec3Graphics start = transform.GetTranslation();
		transform.SetTranslation(Vec3Graphics(0, 0, 0));

		Vec4Graphics endX = transform * Vec4Graphics(1, 0, 0, 1);
		Vec4Graphics endY = transform * Vec4Graphics(0, 1, 0, 1);
		Vec4Graphics endZ = transform * Vec4Graphics(0, 0, 1, 1);

		skeletonVertices[(i * 6) + 0] = currentSkeleton.joints[i].transform.GetTranslation();
		skeletonVertices[(i * 6) + 1] = currentSkeleton.joints[i].transform.GetTranslation() + (endX.ToVector3() * size);

		skeletonVertices[(i * 6) + 2] = currentSkeleton.joints[i].transform.GetTranslation();
		skeletonVertices[(i * 6) + 3] = currentSkeleton.joints[i].transform.GetTranslation() + (endY.ToVector3() * size);
					
		skeletonVertices[(i * 6) + 4] = currentSkeleton.joints[i].transform.GetTranslation();
		skeletonVertices[(i * 6) + 5] = currentSkeleton.joints[i].transform.GetTranslation() + (endZ.ToVector3() * size);


		skeletonColours[(i * 6) + 0] = Vec4Graphics(1, 0, 0, 1);
		skeletonColours[(i * 6) + 1] = Vec4Graphics(1, 0, 0, 1);

		skeletonColours[(i * 6) + 2] = Vec4Graphics(0, 1, 0, 1);
		skeletonColours[(i * 6) + 3] = Vec4Graphics(0, 1, 0, 1);

		skeletonColours[(i * 6) + 4] = Vec4Graphics(0, 0, 1, 1);
		skeletonColours[(i * 6) + 5] = Vec4Graphics(0, 0, 1, 1);
	}

	//You should know what this all does by now, except we combine it with the draw operations in a single function
	glBindVertexArray(skeletonArray);
	glBindBuffer(GL_ARRAY_BUFFER, skeletonBuffer);
	glBufferData(GL_ARRAY_BUFFER, currentSkeleton.numJoints*sizeof(Vec3Graphics) * 6, skeletonVertices, GL_STREAM_DRAW);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0);
	glEnableVertexAttribArray(0);

	glBindBuffer(GL_ARRAY_BUFFER, skeletonColourBuffer);
	glBufferData(GL_ARRAY_BUFFER, currentSkeleton.numJoints*sizeof(Vec4Graphics) * 6, skeletonColours, GL_STREAM_DRAW);
