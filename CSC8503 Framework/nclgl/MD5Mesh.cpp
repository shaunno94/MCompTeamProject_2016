...
void MD5Mesh::BufferExtraData() {
	...
	glBufferData(GL_ARRAY_BUFFER, numVertices*sizeof(Vec2Graphics), weights, GL_STATIC_DRAW);
...
void MD5Mesh::RebufferData()	{
	...
	glBufferSubData(GL_ARRAY_BUFFER, 0, numVertices*sizeof(Vec3Graphics), (void*)vertices);

	if(textureCoords) {
		glBindBuffer(GL_ARRAY_BUFFER, bufferObject[TEXTURE_BUFFER]);
		glBufferSubData(GL_ARRAY_BUFFER, 0, numVertices*sizeof(Vec2Graphics), (void*)textureCoords);
	}

	if (colours)	{
		glBindBuffer(GL_ARRAY_BUFFER, bufferObject[COLOUR_BUFFER]);
		glBufferSubData(GL_ARRAY_BUFFER, 0, numVertices*sizeof(Vec4Graphics), (void*)colours);
	}

#ifdef MD5_USE_NORMALS
	if(normals) {
		glBindBuffer(GL_ARRAY_BUFFER, bufferObject[NORMAL_BUFFER]);
		glBufferSubData(GL_ARRAY_BUFFER, 0, numVertices*sizeof(Vec3Graphics), (void*)normals);
	}
#endif
	...
	if(tangents) {
		...
		glBufferSubData(GL_ARRAY_BUFFER, 0, numVertices*sizeof(Vec3Graphics), (void*)tangents);
	}
#endif
