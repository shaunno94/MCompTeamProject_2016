...
void DebugDrawPS4::Instance()
{
	...
	vertexBuffer = static_cast<btVector3*>	(meshGarlicAllocator.allocate(sizeof(btVector3) * 3000, sce::Gnm::kAlignmentOfBufferInBytes));
	indexBuffer = static_cast<uint32_t*>	(meshGarlicAllocator.allocate(sizeof(uint32_t) * 3000, sce::Gnm::kAlignmentOfBufferInBytes));
}
