...
namespace updaters
{
	...
	void FloorUpdater::update(double dt, particles::ParticleData *p)
	{
		const float localDT = (float)dt;
	...
	void AttractorUpdater::update(double dt, particles::ParticleData *p)
	{
		const float localDT = (float)dt;
	...
	void BasicColorUpdater::update(double dt, particles::ParticleData *p)
	{
		const size_t endId = p->m_countAlive;
		for (size_t i = 0; i < endId; ++i)
	...
	void PosColorUpdater::update(double dt, particles::ParticleData *p)
	{
		const size_t endId = p->m_countAlive;
	...
	void VelColorUpdater::update(double dt, particles::ParticleData *p)
	{
		const size_t endId = p->m_countAlive;
		float scaler, scaleg, scaleb;
	...
	void BasicTimeUpdater::update(double dt, particles::ParticleData *p)
	{
