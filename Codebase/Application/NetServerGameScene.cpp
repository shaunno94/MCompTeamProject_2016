...
NetServerGameScene::NetServerGameScene()
{
}
...
void NetServerGameScene::Setup()
{
	for (int i = NetworkGameData::Instance.server->GetSession()->GetMembers()->size(); i < NetworkGameData::posCount; ++i)
	{
		NetConnectionDataInternal* fakeConnection = new NetConnectionDataInternal(nullptr);
		fakeConnection->name[0] = 0;
		NetworkGameData::Instance.server->AddToSession(fakeConnection);
	}
	...
	NetworkGameData::Instance.server->AddToSession(ballConnection);

	NetworkGameData::Instance.server->StartSession();
	//GameScene::Setup();
	Scene::Setup();
	scores[0] = 0;
	scores[1] = 0;
	...
	player = m_cars[NetworkGameData::Instance.server->GetSessionMemberId()];
	...
	while (!NetworkGameData::Instance.server->GetSession()->GetState() == NetSessionInactive) //waiting for everyone to send the ready message
	{}
	...
	myControllers = new NetServerControllerManager(NetworkGameData::Instance.server->GetSession());
	...
	myControllers->setProducer(ball, NetworkGameData::posCount);
...
void NetServerGameScene::Cleanup()
{
	...
	//NetworkGameData::Instance.server = nullptr;
}
